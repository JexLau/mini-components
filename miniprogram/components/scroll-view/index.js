...
Component({
    ...
    methods: {
        ...
        refresh() {
            ...
            if (this.data.disableRefresh)
                return;
            if (this.data.loading || this.data.refreshLoading)
                return;
            this.setData({
                refreshLoading: true,
                pageIndex: 1,
            ...
            const params = { isRefresh: true, pageIndex: this.data.pageIndex };
            this.triggerEvent("down", params);
        },
        /** 加载新数据 */
        load() {
            console.log("load up");
            if (this.data.disableLoad)
                return;
            const data = this.data;
            if (data.loading || data.refreshLoading || !data.isMoreData)
                return;
            this.setData({
                loading: true
            });
            const params = { isRefresh: false, pageIndex: data.pageIndex };
