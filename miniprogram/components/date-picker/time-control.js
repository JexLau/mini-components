...
Component({
    ...
    methods: {
        ...
        initYear() {
            ...
            const yearLists = this.forDisposeData({
        ...
        initMonth(year) {
            ...
            const monthLists = this.forDisposeData({
                startI,
                endI,
                unit: '月',
                isPulsOne: true
        ...
        initDay(year, month) {
            ...
            const dayLists = this.forDisposeData({
                startI,
                endI,
                unit: '日'
            });
        ...
        initHour(year, month, day) {
            ...
            const hourLists = this.forDisposeData({
                startI,
                endI,
                unit: '时'
            });
        ...
        /** 初始化: 分钟 */
        initMinute(year, month, day, hour) {
            let startI = 0;
            let endI = 59;
            const startDate = new Date(this.data._startDate);
            const endDate = new Date(this.data._endDate);
            ...
            const minuteLists = this.forDisposeData({
                startI,
                endI,
                unit: '分'
            });
        ...
        handlePickerChange(ev) {
            ...
            if (value[0] !== rawData._yearIndex) { // 年
                ...
                this.initDay(rawData.yearLists[rawData._yearIndex].rawValue, rawData.monthLists[rawData._monthIndex].rawValue);
            }
            else if (value[2] !== rawData._dayIndex) { // 日
                rawData._dayIndex = value[2];
                this.initHour(rawData.yearLists[rawData._yearIndex].rawValue, rawData.monthLists[rawData._monthIndex].rawValue, rawData.dayLists[rawData._dayIndex].rawValue);
            }
            else if (value[3] !== rawData._hourIndex) { // 时
                rawData._hourIndex = value[3];
                this.initMinute(rawData.yearLists[rawData._yearIndex].rawValue, rawData.monthLists[rawData._monthIndex].rawValue, rawData.dayLists[rawData._dayIndex].rawValue, rawData.hourLists[rawData._hourIndex].rawValue);
            }
        ...
        /** 初始化指定的时间 */
        initAssignDate(key) {
            if (this.data._isInitAssignDateSuccess)
                return;
            const filterDateData = (dateLists, date) => {
        ...
        /** 获取时间 */
        getDate() {
            const { _yearIndex, _monthIndex, _dayIndex, _hourIndex, _minuteIndex, yearLists, monthLists, dayLists, hourLists, minuteLists } = this.data;
            ...
            return {
                value: (0, util_1.formatTime)(date, 'yyyy-MM-dd hh:mm', false),
